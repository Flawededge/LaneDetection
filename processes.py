...
def binary_pipeline(img, dispAll = False):
    ...
    if dispAll:
        for i in ["1Blur", "2RedBin", "3Sobel", "4Mag", "5Dir", "6Gradient", "7ALL"]:
            cv.namedWindow(i, cv.WINDOW_GUI_NORMAL)
        cv.imshow("1Blur", img_copy)
        cv.imshow("2RedBin", s_binary)
        cv.imshow("3Sobel", xy)
        cv.imshow("4Mag", mag_binary)
        cv.imshow("5Dir", dir_binary)
        cv.imshow("6Gradient", gradient)
        cv.imshow("7ALL", final_binary)
    return final_binary
